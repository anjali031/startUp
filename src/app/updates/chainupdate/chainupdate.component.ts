...
export class ChainupdateComponent implements OnInit {
  ...
  OnSubmit( ORNAMENT_SHOPFOR, WEIGHT , ORNAMENT_CHAIN_STYLE, CAD_FILE , IMAGE_FILE_ONE, IMAGE_FILE_TWO, IMAGE_FILE_THREE) {
    // tslint:disable-next-line: max-line-length
    ...
    this.loading = true;
      // tslint:disable-next-line: max-line-length
    this.userService.chainupdate(ORNAMENT_SHOPFOR.value , WEIGHT.value, this.metala, this.asion, this.asan, this.themea,  this.curateda , this.festia, this.bsizea, ORNAMENT_CHAIN_STYLE.value, this.center , this.stone, this.gem, this.color, this.fileToUpload, this.fileToUploadone, this.fileToUploadtwo, this.fileToUploadthree).subscribe(
      (data: any) => {
        this.loading = false;
