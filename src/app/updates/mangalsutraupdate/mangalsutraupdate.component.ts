...
@Component({
  selector: 'app-mangalsutraupdate',
  templateUrl: './mangalsutraupdate.component.html',
  styleUrls: ['./mangalsutraupdate.component.css']
})
export class MangalsutraupdateComponent implements OnInit {
  ...
  OnSubmit( ORNAMENT_SHOPFOR, WEIGHT , CAD_FILE , IMAGE_FILE_ONE, IMAGE_FILE_TWO, IMAGE_FILE_THREE) {
    // tslint:disable-next-line: max-line-length
    ...
    this.loading = true;
      // tslint:disable-next-line: max-line-length
    this.userService.multiupdate(ORNAMENT_SHOPFOR.value , WEIGHT.value, this.metala, this.asion, this.asan, this.themea, this.curateda , this.festia,  this.center , this.stone, this.gem, this.color, this.fileToUpload, this.fileToUploadone, this.fileToUploadtwo, this.fileToUploadthree).subscribe(
      (data: any) => {
        this.loading = false;
