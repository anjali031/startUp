...
export class PendantupdateComponent implements OnInit {
  ...
  OnSubmit(ORNAMENT_SHOPFOR, WEIGHT , ORNAMENT_PENDANT_STYLE, CAD_FILE , IMAGE_FILE_ONE, IMAGE_FILE_TWO, IMAGE_FILE_THREE) {
    ...
    this.loading = true;
      // tslint:disable-next-line: max-line-length
    this.userService.pendantupdate(ORNAMENT_SHOPFOR.value , WEIGHT.value, this.metala, this.asion, this.asan, this.themea,  this.curateda , this.festia, ORNAMENT_PENDANT_STYLE.value,  this.center , this.stone, this.gem, this.color, this.fileToUpload, this.fileToUploadone, this.fileToUploadtwo, this.fileToUploadthree).subscribe(
      (data: any) => {
        this.loading = false;
